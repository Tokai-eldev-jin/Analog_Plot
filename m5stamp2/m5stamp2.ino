...
#define SERVICE_UUID           "a261407b-14f6-4510-86ed-ddeab74297fb" // UART service UUID
#define CHARACTERISTIC_UUID_RX "0cd118be-e194-4059-a922-c3c91f13c05c"
#define CHARACTERISTIC_UUID_TX "ea7964b4-0681-4328-b5da-4ef3738dec01"
...
void task1(void* arg) {
  ...
  while (1) {
    if(Soushin==1){
      for(ii=0;ii<100;ii++){
        for(i=0;i<3;i++){
          get_val[i][ii]=analogRead(get_ch[i]);
...
void loop() {
    ...
    if (deviceConnected) {
      ...
      MOJI=(String)Soushin_cyc;
      for(i=0;i<100;i++){
        MOJI+=",";
        MOJI+=(String)get_val[Soushin_cyc-3][i];
      }

      MOJI.toCharArray(char_array,1000);
